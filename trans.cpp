...
void singleThreadTest(struct context *ctx){
	if (ctx->id == 0){
		snprintf(req_area[ctx->id], 64,"This is a message in node %d", ctx->id);
		cout << "I have " << req_area[ctx->id] << endl;
	}
	else{
		cout << "Single Thread test" << endl;
		rdma_remote_read(ctx, 0, req_area[ctx->id], req_area_mr[ctx->id]->lkey, req_area_stag[0].buf[0], req_area_stag[0].rkey[0]);
		cout << "Node: " << 0 << " had message " << req_area[ctx->id] << endl;
	}
}
